...
try {
    ...
    $stmt_check = $pdo->prepare("SELECT s.id FROM saude s JOIN gatos g ON s.gato_id = g.id WHERE s.id = ? AND (g.user_id = ? OR ?)");
    $stmt_check->execute([$id, $user_id, is_admin()]);
    if (!$stmt_check->fetch()) {
        json_response(['success' => false, 'message' => 'Acesso negado'], 403);
    }

    $stmt = $pdo->prepare("DELETE FROM saude WHERE id = ?");
    $stmt->execute([$id]);

    json_response(['success' => true, 'message' => 'Evento excluído com sucesso!']);
}
catch (Exception $e) {
    json_response(['success' => false, 'message' => 'Erro ao excluir registro.'], 500);
}
